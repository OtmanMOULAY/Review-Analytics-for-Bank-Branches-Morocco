...
import os
import urllib.parse
from reviews_extraction import extract_reviews_from_url, get_url_list, process_url, combine_csv_files
from reviews_loading import insert_reviews_data_into_postgres
import pandas as pd
...
# Create the DAG
dag = DAG(
    'google_maps_reviews_extraction',
    default_args=default_args,
    description='Extract reviews from Google Maps URLs',
    schedule="@daily",  # Run daily
    start_date=datetime(2023, 1, 1),
    catchup=False,
    tags=['google_maps', 'reviews', 'extraction','sentiment_analysis'],
)
...
# Task 2: Process each URL in parallel
url_tasks = []
urls = get_url_list()
for i, url in enumerate(urls):
    task = PythonOperator(
        task_id=f'process_url_{i}',
        python_callable=process_url,
        op_kwargs={'url': url},
        dag=dag,
    )
    url_tasks.append(task.task_id)
    get_urls_task >> task

# Task 3: Combine all CSV files
combine_task = PythonOperator(
    task_id='combine_csv_files',
    python_callable=combine_csv_files,
    op_kwargs={'task_ids': url_tasks},
    dag=dag,
)



insert_reviews_data_task = PythonOperator(
    task_id='insert_reviews_data',
    python_callable=insert_reviews_data_into_postgres,
    dag=dag,
)

# Task 5: Perform sentiment analysis on inserted reviews
sentiment_analysis_task = PythonOperator(
    task_id='sentiment_analysis',
    python_callable=perform_sentiment_analysis,
    dag=dag,
)
# Set dependencies for the combine task
for i in range(len(urls)):
    dag.get_task(f'process_url_{i}') >> combine_task



combine_task >> insert_reviews_data_task >> sentiment_analysis_task  